...
ret_code_map = dict(
    ...
    MT_RET_AUTH_SERVER_INTERNAL=1007,  # connect to non-access server
    ...
    MT_RET_AUTH_MANAGER_NOCONFIG=1011,  # account doesn't have manager config
    MT_RET_AUTH_MANAGER_IPBLOCK=1012,  # IP address unallowed for manager
    MT_RET_AUTH_GROUP_INVALID=1013,  # Group is not initialized
    MT_RET_AUTH_CA_DISABLED=1014,  # Certificate generation disabled
    MT_RET_AUTH_INVALID_ID=1015,  # Invalid or disabled server id
    MT_RET_AUTH_INVALID_IP=1016,  # Unallowed address
    MT_RET_AUTH_INVALID_TYPE=1017,  # Invalid server type
    ...
    MT_RET_AUTH_SERVER_LIMIT=1022,  # Server license limitation
    MT_RET_AUTH_MOBILE_DISABLED=(
        1023  # Mobile aren't allowed in server license
    ),
    #
    ...
    MT_RET_CFG_NOT_MANAGER_LOGIN=2005,  # account is not from manager group
    ...
    MT_RET_USR_DIFFERENT_SERVERS=3009,  # move account to different server
    MT_RET_USR_DIFFERENT_CURRENCY=3010,  # move account to currency group
    ...
    MT_RET_REPORT_TEMPLATE_END=5005,  # End of template
    ...
    #
    MT_RET_HST_SYMBOL_NOTFOUND=6001,  # Symbol not found
    #
    ...
    MT_RET_REQUEST_DONE_CANCEL=(
        11001  # Request partially filled; remainder has been canceled
    ),
    MT_RET_REQUEST_REQUOTE_RETURN=(
        11002  # Request requoted and returned in queue with new prices
    ),
    #
    # API retcodes
    #
    ...
    MT_RET_SUBS_REAL_ONLY=15007,
)
ret_code_map_by_num = dict(map(lambda x: (x[1], x[0]), ret_code_map.items()))
